...
var vm = require("vm");
var status_message_1 = require("../status-message");
...
var MODULE_WRAP_PREFIX_LENGTH = require('module').wrap('☃').indexOf('☃');
var BreakpointData = (function () {
    function BreakpointData(apiBreakpoint, v8Breakpoint, parsedCondition, 
...
function create(logger_, config_, jsFiles_, sourcemapper_) {
    ...
    var v8Version = /(\d+\.\d+\.\d+)\.\d+/.exec(process.versions.v8);
    if (!v8Version || v8Version.length < 2) {
        return null;
    }
    var usePermanentListener = semver.satisfies(v8Version[1], '>=4.5');
    ...
    singleton = {
        ...
        set: function (breakpoint, cb) {
            if (!v8 || !breakpoint ||
                typeof breakpoint.id === 'undefined' ||
                !breakpoint.location || !breakpoint.location.path ||
                !breakpoint.location.line) {
                return setErrorStatusAndCallback(cb, breakpoint, status_message_1.StatusMessage.UNSPECIFIED, messages.INVALID_BREAKPOINT);
            }
            ...
            if (!sourcemapper.hasMappingInfo(baseScriptPath)) {
                if (!_.endsWith(baseScriptPath, '.js')) {
                    return setErrorStatusAndCallback(cb, breakpoint, status_message_1.StatusMessage.BREAKPOINT_SOURCE_LOCATION, messages.COULD_NOT_FIND_OUTPUT_FILE);
                }
                ...
                if (breakpoint.condition && compile) {
                    try {
                        ...
                        logger
                            .info('Unable to compile condition >> ' + breakpoint.condition +
                            ' <<');
                        return setErrorStatusAndCallback(cb, breakpoint, status_message_1.StatusMessage.BREAKPOINT_CONDITION, messages.ERROR_COMPILING_CONDITION);
                    }
                }
        ...
        wait: function (breakpoint, callback) {
            ...
            var listener = onBreakpointHit.bind(null, breakpoint, function (err) {
        ...
        log: function (breakpoint, print, shouldStop) {
            ...
            var listener = onBreakpointHit.bind(null, breakpoint, function (_err) {
        ...
        messages: messages,
        numBreakpoints_: function () {
            return Object.keys(breakpoints).length;
        },
        numListeners_: function () {
    ...
    function setInternal(breakpoint, mapInfo, compile, cb) {
        ...
        if (breakpoint.condition) {
            try {
                ...
                if (!validator.isValid(ast)) {
                    return setErrorStatusAndCallback(cb, breakpoint, status_message_1.StatusMessage.BREAKPOINT_CONDITION, messages.DISALLOWED_EXPRESSION);
                }
            }
            catch (err) {
                var message = messages.SYNTAX_ERROR_IN_CONDITION + err.message;
                return setErrorStatusAndCallback(cb, breakpoint, status_message_1.StatusMessage.BREAKPOINT_CONDITION, message);
            }
        }
        ...
        if (scripts.length === 0) {
            return setErrorStatusAndCallback(cb, breakpoint, status_message_1.StatusMessage.BREAKPOINT_SOURCE_LOCATION, messages.SOURCE_FILE_NOT_FOUND);
        }
            ...
            return setErrorStatusAndCallback(cb, breakpoint, status_message_1.StatusMessage.BREAKPOINT_SOURCE_LOCATION, messages.SOURCE_FILE_AMBIGUOUS);
        }
        // TODO: Address the case where `breakpoint.location` is `null`.
        // TODO: Address the case where `fileStats` is `null`.
        // TODO: Address the case where `fileStats[matchingScript]` is `null`.
        if (breakpoint.location.line >=
            fileStats[matchingScript].lines) {
            return setErrorStatusAndCallback(cb, breakpoint, status_message_1.StatusMessage.BREAKPOINT_SOURCE_LOCATION, messages.INVALID_LINE_NUMBER + matchingScript + ':' +
                breakpoint.location.line +
        ...
        if (!v8bp) {
            return setErrorStatusAndCallback(cb, breakpoint, status_message_1.StatusMessage.BREAKPOINT_SOURCE_LOCATION, messages.V8_BREAKPOINT_ERROR);
        }
        if (numBreakpoints === 0 && !usePermanentListener) {
    ...
    function getBreakpointCompiler(breakpoint) {
        // TODO: Address the case where `breakpoint.location` is `null`.
        switch (path.normalize(breakpoint.location.path)
            .split('.')
            .pop()) {
            case 'coffee':
    ...
    function onBreakpointHit(breakpoint, callback, execState) {
        ...
        if (!v8bp.active()) {
            ...
            return setErrorStatusAndCallback(callback, breakpoint, status_message_1.StatusMessage.BREAKPOINT_SOURCE_LOCATION, messages.V8_BREAKPOINT_DISABLED);
        }
        var result = checkCondition(breakpoint, execState);
        if (result.error) {
            return setErrorStatusAndCallback(callback, breakpoint, status_message_1.StatusMessage.BREAKPOINT_CONDITION, messages.ERROR_EVALUATING_CONDITION + result.error);
        }
        ...
        try {
            ...
            return setErrorStatusAndCallback(callback, breakpoint, status_message_1.StatusMessage.BREAKPOINT_SOURCE_LOCATION, messages.CAPTURE_BREAKPOINT_DATA + err);
        }
    ...
    function captureBreakpointData(breakpoint, execState) {
        ...
        if (breakpoint.expressions &&
            breakpoints[breakpoint.id].compile) {
            for (var i = 0; i < breakpoint.expressions.length; i++) {
                try {
                    ...
                    // TODO: Address the case where `logger` is `null`.
                    logger
                        .info('Unable to compile watch expression >> ' +
                        breakpoint.expressions[i] + ' <<');
...
function findScripts(scriptPath, config, fileStats) {
    ...
    if (config.appPathRelativeToRepository) {
        var candidate = scriptPath.replace(
...
//# sourceMappingURL=v8debugapi.js.map
