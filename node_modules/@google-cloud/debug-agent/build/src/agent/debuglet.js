...
var v8debugapi = require("./v8debugapi");
...
var Debuglet = (function (_super) {
    ...
    function Debuglet(debug, config) {
        ...
        /** @private {object} */
        _this.config_ = Debuglet.normalizeConfig_(config);
        /** @private {Debug} */
        _this.debug_ = debug;
        /**
        ...
        _this.project_ = null;
        /** @private {boolean} */
        _this.fetcherActive_ = false;
        /** @private {common.logger} */
        _this.logger_ = new common.logger({ level: common.logger.LEVELS[_this.config_.logLevel], tag: pjson.name });
        /** @private {DebugletApi} */
        _this.debugletApi_ = new controller_1.Controller(_this.debug_);
        /** @private {Debuggee} */
        _this.debuggee_ = null;
        /** @private {Object.<string, Breakpoint>} */
        _this.activeBreakpointMap_ = {};
        /** @private {Object.<string, Boolean>} */
        _this.completedBreakpointMap_ = {};
        return _this;
    }
    Debuglet.normalizeConfig_ = function (config) {
        ...
        return extend(true, {}, config_1.default, config, envConfig);
    };
    /**
    ...
    Debuglet.prototype.start = function () {
        return __awaiter(this, void 0, void 0, function () {
            var that, stat, err_1, id, fileStats, err_2, jsStats, mapFiles;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        //       be `null`.
                        return [4 /*yield*/, stat(path.join(that.config_.workingDirectory, 'package.json'))];
                    case 2:
                        // TODO: Address the fact that `that.config_.workingDirectory` could
                        //       be `null`.
                        _a.sent();
                    ...
                    case 5:
                        _a.trys.push([5, 7, , 8]);
                        return [4 /*yield*/, scanner.scan(!id, that.config_.workingDirectory, /.js$|.map$/)];
                    case 6:
                    ...
                    case 7:
                        err_2 = _a.sent();
                        that.logger_.error('Error scanning the filesystem.', err_2);
                        that.emit('initError', err_2);
                        return [2 /*return*/];
                    case 8:
                        jsStats = fileStats.selectStats(/.js$/);
                        mapFiles = fileStats.selectFiles(/.map$/, process.cwd());
                        SourceMapper.create(mapFiles, function (err3, mapper) {
                            return __awaiter(this, void 0, void 0, function () {
                                var onGCP, project, err_3, clusterName, err_4;
                                return __generator(this, function (_a) {
                                    switch (_a.label) {
                                        case 0:
                                            if (err3) {
                                        ...
                                        case 2:
                                            _a.trys.push([2, 4, , 5]);
                                            return [4 /*yield*/, Debuglet.getProjectId(that.debug_.options)];
                                        case 3:
                                            project = _a.sent();
                                            return [3 /*break*/, 5];
                                        case 4:
                                            err_3 = _a.sent();
                                            that.logger_.error(err_3.message);
                                            that.emit('initError', err_3);
                                            return [2 /*return*/];
                                        case 5:
                                            if (!(onGCP &&
                                                (!that.config_.serviceContext ||
                                                    !that.config_.serviceContext.service))) return [3 /*break*/, 9];
                                            _a.label = 6;
                                        case 6:
                                            _a.trys.push([6, 8, , 9]);
                                        ...
                                        case 9:
                                            that.getSourceContext_(function (err5, sourceContext) {
                                                ...
                                                project, id, that.config_.serviceContext, sourceContext, that.config_.description, null, onGCP);
    ...
    Debuglet.createDebuggee = function (projectId, uid, serviceContext, sourceContext, description, errorMessage, onGCP) {
        var cwd = process.cwd();
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, lookup('metadata.google.internal.')];
                    case 2:
                        _a.sent();
    ...
    Debuglet.getProjectIdFromMetadata = function () {
        return new Promise(function (resolve, reject) {
            metadata.project('project-id', function (err, _res, projectId) {
    ...
    Debuglet.getClusterNameFromMetadata = function () {
        return new Promise(function (resolve, reject) {
            metadata.instance('attributes/cluster-name', function (err, _res, clusterName) {
    ...
    Debuglet.prototype.scheduleRegistration_ = function (seconds) {
        ...
        function onError(err) {
            that.logger_.error('Failed to re-register debuggee ' + that.project_ + ': ' + err);
            that.scheduleRegistration_(Math.min((seconds + 1) * 2, that.config_.internal.maxRegistrationRetryDelay));
        }
        setTimeout(function () {
            if (!that.running_) {
    ...
    Debuglet.prototype.scheduleBreakpointFetch_ = function (seconds) {
        ...
        setTimeout(function () {
            ...
            that.debugletApi_.listBreakpoints(that.debuggee_, function (err, response, body) {
                if (err) {
                    that.logger_.error('Unable to fetch breakpoints – stopping fetcher', err);
                    ...
                    // breakpoint fetcher.
                    that.scheduleRegistration_(that.config_.internal.registerDelayOnFetcherErrorSec);
                    return;
                }
                ...
                switch (response.statusCode) {
                    ...
                    default:
                        // TODO: Address the case where `response` is `undefined`.
                        that.logger_.info('\t' + response.statusCode +
                            ' completed.');
                        if (!body) {
                        ...
                        if (Object.keys(that.activeBreakpointMap_).length) {
                            that.logger_.info(formatBreakpoints('Active Breakpoints: ', that.activeBreakpointMap_));
                        }
                        that.scheduleBreakpointFetch_(that.config_.breakpointUpdateIntervalSec);
                        return;
                }
    ...
    Debuglet.prototype.updateActiveBreakpoints_ = function (breakpoints) {
        ...
        if (breakpoints.length) {
            that.logger_.info(formatBreakpoints('Server breakpoints: ', updatedBreakpointMap));
        }
        ...
        Debuglet.mapSubtract(this.completedBreakpointMap_, updatedBreakpointMap)
            ...
            delete that.completedBreakpointMap_[breakpoint.id];
        });
    ...
    Debuglet.prototype.rejectBreakpoint_ = function (breakpoint) {
        var that = this;
        // TODO: Address the case when `that.debuggee_` is `null`.
        that.debugletApi_.updateBreakpoint(that.debuggee_, breakpoint, function (err /*, body*/) {
            if (err) {
                that.logger_.error('Unable to complete breakpoint on server', err);
            }
        });
