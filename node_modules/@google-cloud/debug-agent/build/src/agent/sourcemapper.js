...
var v8debugapi_1 = require("./v8debugapi");
...
function processSourcemap(infoMap, mapPath, callback) {
    ...
    if (!mapPath || !_.endsWith(mapPath, MAP_EXT)) {
        return setImmediate(function () {
            callback(new Error('The path ' + mapPath + ' does not specify a sourcemap file'));
        });
    }
    mapPath = path.normalize(mapPath);
    fs.readFile(mapPath, 'utf8', function (err, data) {
        if (err) {
            return callback(new Error('Could not read sourcemap file ' + mapPath + ': ' + err));
        }
        var consumer;
        try {
         ...
         */
        var outputBase = consumer.file ? consumer.file : path.basename(mapPath, '.map');
        ...
        if (sources.length === 0) {
            return callback(new Error('No sources listed in the sourcemap file ' + mapPath));
        }
...
var SourceMapper = (function () {
     ...
     */
    SourceMapper.prototype.getMappingInfo = function (inputPath) {
        if (this.infoMap_.has(path.normalize(inputPath))) {
            return this.infoMap_.get(inputPath);
        }
        var matches = v8debugapi_1.findScriptsFuzzy(inputPath, Array.from(this.infoMap_.keys()));
        if (matches.length === 1) {
            return this.infoMap_.get(matches[0]);
        }
        return null;
    };
    /**
    ...
    SourceMapper.prototype.mappingInfo = function (inputPath, lineNumber, colNumber) {
        ...
        var entry = this.getMappingInfo(inputPath);
        if (entry === null) {
            return null;
        }
        var sourcePos = {
        ...
        var mappedPos = allPos && allPos.length > 0 ?
            Array.prototype.reduce.call(allPos, function (accumulator, value /*, index, arr*/) {
...
function create(sourcemapPaths, callback) {
    ...
    var callList = Array.prototype.slice.call(sourcemapPaths).map(function (p) {
        return function (cb) {
    ...
    async.parallelLimit(callList, 10, function (err) {
        if (err) {
            return callback(new Error('An error occurred while processing the sourcemap files' + err));
        }
